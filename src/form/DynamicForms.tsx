...
function DynamicForms({cuestionarios, currentFormIndex, onNext}: DynamicFormProps) {
        ...
        currentForm.preguntas.forEach((pregunta) => {
            ...
            if (pregunta.tipo == "text" || pregunta.tipo == "textarea") {
                ...
                if (pregunta.validacion) {
                    if (pregunta.validacion.formato === 'email' && !validateEmailFormat(stringValue)) {
                        valid = false;
                        return;
                    }
                    ...
                    if (pregunta.id === 'fecha_nacimiento') {
                        ...
                        if (pregunta.validacion.min_edad && !validateAge(stringValue, pregunta.validacion.min_edad)) {
                            valid = false;
                            return;
                        }
                    }
                ...
                if (pregunta.validacion?.max_seleccionados && !validateMaxSelected<string | number>(arrayValue, pregunta.validacion.max_seleccionados)) {
                    valid = false;
                    return;
                }
            }
            ...
            case "text":
                return (
                    <Input
                        ...
                        name={pregunta.id}
                        placeholder={pregunta.id === 'fecha_nacimiento' || pregunta.id === 'fecha' ? "mm/dd/yyyy" : "Escribe un texto"}
            ...
            case "check":
                return (
                    <Checkbox
                        ...
                        label={pregunta.pregunta}
                        options={pregunta.opciones || []}
                        onSelectOption={(option) => handleCheckboxChange(pregunta.id, option)}
                        selectedOptions={Array.isArray(formData[pregunta.id]) ? formData[pregunta.id] as string[] : []}
                        rules={[
                            { message: "Este campo es obligatorio*" },
        ...
        if (pregunta.restricciones) {
            if (pregunta.restricciones.min && pregunta.restricciones.max) {
                rules.push({
                    ...
                    isValid: validateLength(value, pregunta.restricciones.min, pregunta.restricciones.max)
                })
            ...
            if (pregunta.id === 'fecha_nacimiento') {
                ...
                if (pregunta.validacion.min_edad) {
                    rules.push({
                        ...
                        isValid: validateBirthDateFormat(value) && validateAge(value, pregunta.validacion.min_edad)
                    })
                }
            }
        }
    ...
    return (
        <div className="flex flex-col gap-5 w-99 p-6 border rounded-3xl">
            {currentForm.preguntas.map((pregunta) => (
                <div key={pregunta.id}>
                    { renderFormField(pregunta) }
                </div>
            ))}
            <Button enabled={isFormValid} onClick={onNext} />
        </div>
    )
}
